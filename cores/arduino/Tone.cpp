...
void tone(const pin_t _pin, const uint32_t frequency, const uint32_t duration = 0) {
  ...
  if (!initialised_tone_timer) {
    LPC176x::util::bit_set(LPC_SYSCTL->PCONP, 22);                     // Power ON Timer 2
    LPC_TIMER2->TCR = LPC176x::util::bit_value(1);                   // Put Timer in reset state
    LPC_TIMER2->PR = ((SystemCoreClock / 4) / 1000000) - 1; // Use prescaler to set frequency
    LPC_TIMER2->MCR = LPC176x::util::bitset_value(0, 1);             // Match on MR0, reset on MR0, interrupts when NVIC enables them
...
extern "C" [[gnu::optimize("O3")]] void TIMER2_IRQHandler(void) {
  const uint32_t interrupts = LPC_TIMER2->IR;
  LPC_TIMER2->IR = interrupts;  // clear all interrupts
  if (toggles != 0) {
